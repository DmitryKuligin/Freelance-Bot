...
def make_request(name, description, cost):
    cost = int(cost)
    query = f"insert into tasks values ('{name}', '{description}', {cost}, NULL, 0)"
    cursor.execute(query)
    connection.commit()

def show_tasks(username):
    query = f"select * from tasks where status = 1 and theme = select theme fro users where username = '{username}'"
    cursor.execute(query)
